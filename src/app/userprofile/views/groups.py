...
def delete_group(request, group_id):
    try:
        ...
        messages.warning(
            request, 'The group has already been deleted. You may have clicked twice.')
...
def unregister_user_from_group(request, group_id, user_id):
    ...
    messages.success(request, 'Successfully removed %s from %s.' %
                     (user, group))
...
def generate_user_for_group(request, group_id):
    ...
    user.groups.add(group)
    messages.success(
        request, 'Successfully generated a user for %s.' % group.name)
    return redirect(view_group, group_id)
...
def remove_user_from_group(request, group_id, user_id):
    try:
        ...
        user.groups.remove(group)
        messages.success(
            request, 'Successfully removed user from %s.' % group.name)
    except ObjectDoesNotExist:
        messages.warning(
            request, 'The user has already been removed. You may have clicked twice.')
    return redirect(view_group, group_id)
